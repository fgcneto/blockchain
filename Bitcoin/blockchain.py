...
class Blockchain(object):
    ...
    def isValidChain(self, chain):
        ...
        while current_index < len(chain):
            ...
            if not self.isValidChain(last_block['nonce'], block['nonce']):
                return False
    ...
    def resolveConflicts(self):
        ''' Consulta todos os nós registrados, e verifica se algum outro 
            nó tem um blockchain mais comprido e válido. Em caso positivo, 
        ...
        # Verifica o tamanho da chain
        max_length = len(self.chain)

        # verifica as chains de todos os nós na rede
        for node in vizinhos:

            # utiliza a propria api para listar e construir uma lista de chains
            ...
            if response.status_code == 200:
                ...
                # Verifica se a chain é a mais longa e válida
                if length > max_length and self.valid_chain(chain):
                    max_length = length
                    new_chain = chain

        # substitui a chain se encontrar uma no chain mais longa
        if new_chain:
    ...
    def createTransaction(self, sender, recipient, amount, timestamp, privWifKey):
        ...
        tx['signature'] = Blockchain.sign(
            privWifKey, json.dumps(tx, sort_keys=True))
    ...
    def _hashTxHashes(txHashes):
        ...
        for i in range(0, len(txHashes), 2):
            newTxHashes.append(Blockchain.generateHash(
